-- CREDIT https://github.com/SuperChocolatine20/ElectricSkateboardFiveM
local RCCar = {}
	...
	if distanceCheck <= Config.LoseConnectionDistance then
		...
		if distanceCheck <= 1.5 then
			...
			if not Attached then
				DrawText3Ds(GetEntityCoords(RCCar.Entity).x, GetEntityCoords(RCCar.Entity).y, GetEntityCoords(RCCar.Entity).z + 0.5, "~r~[E] เก็บ ~g~[G] ขึ้น")
			end
		end
	end

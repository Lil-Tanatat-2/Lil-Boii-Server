	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not IsPickingUp then
				ESX.ShowHelpNotification("Press ~INPUT_CONTEXT~ to ~o~Dig a Crab.~s~")
			end

			if IsControlJustReleased(0, Config.Key['E']) and not IsPickingUp then
				ESX.TriggerServerCallback("meeta_crab:checkItem", function(result, itemfull)
					if result == true then 

						if itemfull then
							TriggerEvent("pNotify:SendNotification", {
								text = '<strong class="red-text">ปูม้าคุณเต็ม</strong>',
								type = "error",
								timeout = 3000,
								layout = "bottomCenter",
								queue = "global"
							...
							TriggerServerEvent('meeta_crab:pickedUp')

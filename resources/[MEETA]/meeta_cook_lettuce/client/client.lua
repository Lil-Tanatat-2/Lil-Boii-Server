	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if not IsPickingUp then
				ESX.ShowHelpNotification("Press ~INPUT_CONTEXT~ to pick ~g~Lettuce.~s~")
			end

			if IsControlJustReleased(0, Config.Key['E']) and not IsPickingUp then
				ESX.TriggerServerCallback("meeta_cook_lettuce:checkItem", function(result, itemfull)
					if result == true then 
						...
						if itemfull then
						  ...
						  function(status)
							...
							TriggerServerEvent('meeta_cook_lettuce:pickedUp')

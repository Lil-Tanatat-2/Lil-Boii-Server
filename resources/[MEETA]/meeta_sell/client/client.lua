...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(0, Config.Key['F9']) then
				...
				ESX.TriggerServerCallback("meeta_selling:getPriceItems", function(data)
					...
					for k,v in pairs(data) do
						...
						if IsRandomPrice(v.item) then
							if v.price <= 0 then
								TextPrice = ('%s <span class="red-text">%s</span> '):format(v.label, "ปิดการขาย")
							else
			...
			if inventory ~= nil then
				...
				if Count > 0 then 
					...
					if CurrentZone.Item[i].ListItem then
						...
						local data = {
							label     = "เสนอขาย ".. CurrentZone.Item[i].Text_TH .." <strong class='blue-text'>จำนวน 1 ".. CurrentZone.Item[i].Unit .. "</strong>",
							name      = i,
							value     = 1,
							zone 	 = CurrentZone,
						}
...
function AddMenuConfirmSell(ItemName, ItemCount, ItemPrice)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'sell_dialog_confirm', {
		title    = "ร้านค้าได้เสนอราคาให้คุณ <strong class='green-text'>จำนวน $"..ItemPrice.."</strong>",
		align    = 'top-left',
		elements = {
			{
				label = "ตกลง",

...
AddEventHandler('meeta_selling:sellFunction', function(Index, CurrentZone, Count)
	...
	if Price > 0 then
		...
		TriggerClientEvent("pNotify:SendNotification", _source, {
			text = '<strong class="red-text">ปิดร้านชั่วคราว</strong> ',
			type = "error",
			timeout = 3000,
			layout = "bottomCenter",
			queue = "global"
...
AddEventHandler('meeta_selling:confirmToSell', function(ItemName, ItemCount, ItemPrice)
	...
	if xItem.count <= 0 then
		...
		TriggerClientEvent("pNotify:SendNotification", _source, {
			text = 'คุณได้ขาย <strong class="yellow-text">'..xItem.label..' x1</strong> ได้รับ <strong class="green-text">$'..ItemPrice..'</strong> ',
			type = "success",
			timeout = 3000,
			layout = "bottomCenter",
			queue = "global"

...
AddEventHandler('loffe_robbery:msgPolice', function(store, robber)
    local targetPlayer = GetPlayerPed(GetPlayerFromServerId(robber))
    ...
    while true do
        ...
        --     blipPed = AddBlipForEntity(targetPlayer) 

        --     SetBlipSprite(blipPed, 1)
                    ...
                    if HasEntityClearLosToEntityInFront(me, peds[i], 19) and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 5.0 then
                        if not robbing then
                            ...
                            if canRob == true then
                                ...
                                TriggerServerEvent('loffe_robbery:callCop', i)
                                local scared = 0

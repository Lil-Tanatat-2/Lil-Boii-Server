    ...
    function(data, menu)
		...
		if data.current.value == 'drag' then			
			...
			TriggerEvent("pNotify:SendNotification", {
				text = '<span class="white-text">กำลังอุ้มบางคน...</span>',
				type = "error",
				timeout = 3000,
				layout = "bottomCenter",
				queue = "global"
			...
			if hasRope == true then
				...
				local IsPedRagdoll = IsPedRagdoll(closestPlayerPed)
				...
				if IsPedDeadOrDying(closestPlayerPed, 1) then
					if distance ~= -1 and distance <= 3.0 then
						...
						TriggerEvent("pNotify:SendNotification", {
							text = '<span class="yellow-text">ไม่มีผู้เล่นที่อยู่ใกล้คุณ...</span>',
							type = "error",
							timeout = 3000,
							layout = "bottomCenter",
							queue = "global"
					...
					if distance ~= -1 and distance <= 3.0 then
						if isCarry or IsPedRagdoll then
							local closestPlayer, distance = ESX.Game.GetClosestPlayer()
							TriggerServerEvent('esx_barbie_lyftupp:lyfter', GetPlayerServerId(closestPlayer))		
							...
							TriggerEvent("pNotify:SendNotification", {
								text = 'คุณไม่สามารถ<strong class="red-text">อุ้ม</strong>ได้',
								type = "error",
								timeout = 3000,
								layout = "bottomCenter",
								queue = "global"
...
function OpenActionMenuInteraction1(target)
...
RegisterNetEvent('esx_barbie_lyftupp:on')
AddEventHandler('esx_barbie_lyftupp:on', function()
	OpenActionMenuInteraction1()
end)

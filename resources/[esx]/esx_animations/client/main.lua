...
function startAttitude(lib, anim)
	TriggerEvent("meeta_drunk:setWalk", lib, anim)
...
function OpenAnimationsSubMenu(menu)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'animations_sub',
	{
		...
		local object = data.current.value

		if type == 'scenario' then
			if anim == "PROP_HUMAN_SEAT_BENCH" then
				local playerPed = GetPlayerPed(-1)
				local pos = GetEntityCoords(playerPed)
				TaskStartScenarioAtPosition(playerPed, anim, pos.x, pos.y, pos.z-object.offset, GetEntityHeading(playerPed), 0, true, true)
			else
				TaskStartScenarioInPlace(PlayerPedId(), anim, 0, false)
			end
		...
		elseif type == 'face' then
			SetFacialIdleAnimOverride(GetPlayerPed(-1), anim)
		end
	end, function(data, menu)
		menu.close()
	end)

...
ESX.RegisterServerCallback('esx_policejob:getHouseByName', function(source, cb, name)
	...
	MySQL.Async.fetchAll('SELECT * FROM users WHERE firstname = @firstname AND lastname = @lastname', {
		...
		if result[1] ~= nil then
			...
			MySQL.Async.fetchAll("SELECT * FROM properties_player WHERE owner = @identifier", {
				['@identifier'] = result[1].identifier,
			}, function(result_player)
		
				MySQL.Async.fetchAll('SELECT * FROM owned_properties WHERE id = @name',  {
					['@name'] = result_player[1].properties_id
				}, function(result2)

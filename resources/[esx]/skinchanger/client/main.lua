...
function ApplySkin(skin, clothes)
	...
	if Config.UseFixBug then

		local torso_1 = Character['torso_1']
		local torso_2 = Character['torso_2']

		local tshirt_1 = Character['tshirt_1']
		local tshirt_2 = Character['tshirt_2']

		local pants_1 = Character['pants_1']
		local pants_2 = Character['pants_2']

		local shoes_1 = Character['shoes_1']
		local shoes_2 = Character['shoes_2']

		local arms_1 = Character['arms']
	
		local Default_Shirt_No_Hand
		local ConfigShirt = {}
		local ConfigUnderShirt = {}
		local UnShirtLongHandArm = {}

		if LastSex == 0 then -- For Male
			ConfigShirt = Config.Shirt_M
			ConfigUnderShirt = Config.UnderShirt_M
			UnShirtLongHandArm = Config.UnShirtLongHandArm
			Default_Shirt_No_Hand = 15
		else -- For Female
			ConfigShirt = Config.Shirt_F
			ConfigUnderShirt = Config.UnderShirt_F
			UnShirtLongHandArm = Config.UnShirtLongHandArm_F
			Default_Shirt_No_Hand = 3
		end

		for k,v in pairs(ConfigShirt) do
			if has_value(v.Shirt , torso_1) then

				SetPedComponentVariation(playerPed, 11, torso_1, torso_2, 2)
				SetPedComponentVariation(playerPed, 8, tshirt_1, 0, 2)
				if v.UnderShirt ~= nil then

					SetPedComponentVariation(playerPed, 8, v.UnderShirt[1], 0, 2)

					-- Character['tshirt_1'] = v.UnderShirt[1]
					-- Character['tshirt_2'] = 0

					if has_value(v.UnderShirt , tshirt_1) then
							
						SetPedComponentVariation(playerPed, 8, tshirt_1, tshirt_2, 2)

						for kk,vv in pairs(ConfigUnderShirt) do

							if has_value(vv.Shirt , tshirt_1) then

								if v.IsLongHand then
									SetPedComponentVariation(playerPed, 3, vv.Arm_Long[1], 0, 2)
								elseif v.IsSet and tshirt_1 == Default_Shirt_No_Hand then
									SetPedComponentVariation(playerPed, 3, v.Arm[1], 0, 2)
								else
									SetPedComponentVariation(playerPed, 3, vv.Arm[1], 0, 2)

									if has_value(vv.Arm , arms_1) then
										SetPedComponentVariation(playerPed, 3, arms_1, 0, 2)
									else
										SetPedComponentVariation(playerPed, 3, vv.Arm[1], 0, 2)
									end
						...
						if v.IsLongHand then
							SetPedComponentVariation(playerPed, 3, UnShirtLongHandArm[1], 0, 2)
						else
							
							SetPedComponentVariation(playerPed, 3, v.Arm[1], 0, 2)

							if has_value(v.Arm , arms_1) then
								SetPedComponentVariation(playerPed, 3, arms_1, 0, 2)
							else
								SetPedComponentVariation(playerPed, 3, v.Arm[1], 0, 2)
							end
					...
					SetPedComponentVariation(playerPed, 8, Default_Shirt_No_Hand, 0, 2)
				...
				-- Set Pant
				if v.Pant ~= nil then
					if has_value(v.Pant , pants_1) then
						SetPedComponentVariation(playerPed, 4, pants_1, pants_2, 2)
					else
						SetPedComponentVariation(playerPed, 4, v.Pant[1], pants_2, 2)
					end
				else
					SetPedComponentVariation(playerPed, 4, pants_1, pants_2, 2)
				end
				...
				if v.Shoes ~= nil then
					if has_value(v.Shoes , shoes_1) then
						SetPedComponentVariation(playerPed, 6, shoes_1, shoes_2, 2)
					else
						SetPedComponentVariation(playerPed, 6, v.Shoes[1], shoes_2, 2)
					end
				else
					SetPedComponentVariation(playerPed, 6, shoes_1, shoes_2, 2)
				end

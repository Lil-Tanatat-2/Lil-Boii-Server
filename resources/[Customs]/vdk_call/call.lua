...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(1, 73) and callActive then
            ...
            SendNotification("You took the call")
        ...
        elseif IsControlJustPressed(1, 249) and callActive then
            SendNotification("You refused the call")
            callActive = false
        end
        if haveTarget then
...
AddEventHandler("call:callIncoming", function(job, pos)
    ...
    work = job
    target.pos = pos
    SendNotification("Press on ~g~X~s~ to take the call or ~g~N~s~ to refuse")
end)

RegisterNetEvent("call:taken")
AddEventHandler("call:taken", function()
    callActive = false
    SendNotification("L'call was taken")
end)
...
AddEventHandler("target:call:taken", function(taken)
    ...
    elseif taken == 2 then
        SendNotification("Please remember in a moment")
    end
end)

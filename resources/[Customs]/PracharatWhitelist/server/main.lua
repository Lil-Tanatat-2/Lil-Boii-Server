...
AddEventHandler('playerDropped', function(reason)
	...
	if(inConnection[_source] ~= nil) then
		inConnection[_source] = false -- table.remove(inConnection, _source)
	end
...
AddEventHandler("playerConnecting", function(playerName, reason, deferrals)
	...
	if not found then
		reason(_U("not_in_whitelist", Config.CommunityLink, steamID))
		deferrals.done(_U("not_in_whitelist", Config.CommunityLink, steamID))
		CancelEvent()
...
RegisterServerEvent("xDWL:removePlayerToInConnect")
AddEventHandler("xDWL:removePlayerToInConnect", function()
	local _source = source
	if _source ~= nil and inConnection[_source] ~= nil then
...
TriggerEvent('es:addGroupCommand', 'addwl', 'admin', function (source, args, user)
	if not args[1] or not args[2] then
		TriggerEvent('xDWL:sendMessage', source, '^1SYSTEM', 'Invalid usage!')
		return
	end

	if args[1] == 'hex' then
		if string.len(args[2]) == 21 then
			TriggerEvent('xDWL:whitelistUser', source, args[2])
		else
			TriggerEvent('xDWL:sendMessage', source, '^1SYSTEM', 'Invalid steam hex length!')
		end
	elseif args[1] == 'dec' then
		if tonumber(args[2]) and string.len(args[2]) == 17 then
			TriggerEvent('xDWL:whitelistUser', source, ConvertDecToHex(tonumber(args[2])))
		else
			TriggerEvent('xDWL:sendMessage', source, '^1SYSTEM', 'Invalid steam dec length!')
		end
	else
		TriggerEvent('xDWL:sendMessage', source, '^1SYSTEM', 'Invalid usage, unknown numeral system!')
	end
end, function (source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficienct permissions!' } })
...
AddEventHandler('xDWL:whitelistUser', function(source, identifier)
	MySQL.Async.fetchAll('SELECT * FROM whitelist WHERE identifier=@identifier', {['@identifier'] = identifier}, function(result)
		if result[1] ~= nil then
			TriggerEvent('xDWL:sendMessage', source, '^1SYSTEM', 'The player is already whitelisted on this server!')
		else
			MySQL.Async.execute("INSERT INTO whitelist (identifier) VALUES (@identifier)", {
				['@identifier'] = identifier
			}, function(rowsChanged)
				TriggerEvent('xDWL:sendMessage', source, 'Whitelist', 'The player has been whitelisted! Identifier: ' .. identifier)
				loadWhiteList()
			end)
		end
	end)
...
AddEventHandler('xDWL:sendMessage', function(source, title, message)
	if source ~= 0 then
